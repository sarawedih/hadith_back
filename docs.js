module.exports = (req, res, next) => {
  res.json({
    ...
    endpoints: [
      ...
      //Ajouter l'endpoint de hadiths dans un livre 
      {
        endpoint: "/v1/site/book/:id/hadiths",
        example: "/v1/site/book/10/hadiths",
        description: "جلب جميع الأحاديث الموجودة في كتاب معين.",
        method: "GET",
        params: {
            id: "معرف الكتاب الذي سيتم جلب الأحاديث الخاصة به"
        },
        abstractResponse: {
            status: "success",
            data: [
                {
                    id: "معرف الحديث",
                    idInBook: "رقم الحديث في الكتاب",
                    bookId: "معرف الكتاب",
                    chapterId: "رقم الفصل",
                    arabic: "نص الحديث باللغة العربية",
                    english: {
                        narrator: "اسم الراوي",
                        text: "نص الحديث باللغة الإنجليزية"
                    }
                }
            ]
        }
      ...
      //Ajouter l'endpoint de recherche des hadiths dans un livre 
      {
        endpoint: "/v1/site/book/:id/hadiths/search",
        example: "/v1/site/book/10/hadiths/search?query=Actions",
        description: "البحث عن الأحاديث داخل كتاب معين باستخدام كلمة مفتاحية.",
        method: "GET",
        queryParams: {
            query: "الكلمة المراد البحث عنها في الأحاديث"
        },
        abstractResponse: {
            status: "success",
            count: "عدد الأحاديث التي تم العثور عليها",
            data: [
                {
                    id: "معرف الحديث",
                    idInBook: "رقم الحديث في الكتاب",
     ...
     //Ajouter les endpoints des quiz de Sira et Hadith

     {
      endpoint: "/api/quiz_hadith/start/:level",
      example: "/api/quiz_hadith/start/1",
      description: "بدء اختبار حول الأحاديث بناءً على المستوى. يجب أن يكون المستوى بين 1 و 10.",
      method: "GET",
      abstractResponse: {
  ...
  {
      endpoint: "/api/quiz_hadith/submit",
      example: "/api/quiz_hadith/submit",
      description: "إرسال إجابات الاختبار وحساب النتيجة",
      method: "POST",
      requestBody: {
          sessionId: "ID de la session",
          answers: [
              { id: "ID de la question", answer: "الإجابة المختارة" }
          ]
      },
      abstractResponse: {
          sessionId: "ID de la session",
          score: "النتيجة المحققة",
          total: "إجمالي عدد الأسئلة",
          incorrectAnswers: [
              {
                  id: "ID de la question",
                  question: "نص السؤال",
                  userAnswer: "الإجابة التي قدمها المستخدم",
                  correctAnswer: "الإجابة الصحيحة"
              }
          ],
          message: "رسالة تحدد ما إذا كان يمكن للمستخدم الانتقال إلى المستوى التالي"
      }
  },
  {
      endpoint: "/api/quiz_sire/start/:level",
      example: "/api/quiz_sire/start/1",
      description: "بدء اختبار حول السيرة بناءً على المستوى. يجب أن يكون المستوى بين 1 و 10.",
      method: "GET",
      abstractResponse: {
          sessionId: "ID unique de la session",
          level: "المستوى المحدد",
  ...
  {
      endpoint: "/api/quiz_sire/submit",
      example: "/api/quiz_sire/submit",
      description: "Soumet les réponses du quiz et calcule le score.",
      method: "POST",
      requestBody: {
          sessionId: "ID de la session",
          answers: [
